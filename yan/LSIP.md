...
　　LSIC和LSIP这两个概念非常相似，一个是硬件，一个是软件。单个晶体管的结构很简单，三极管只有 B、C、E 三个极；单个超链接也只有 超文本、href/链接网址、title/提示文字 3个核心语法段。单个晶体管的功能很单纯：实现电流增益；单个超链接的功能也很单纯：实现文件跳转。　集成电路是在一块小型晶片上，把很大数量的微晶体管集成在一起，形成某种功能的电气回路。
